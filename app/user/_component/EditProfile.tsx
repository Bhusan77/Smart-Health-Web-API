...
export default function EditUserForm() {
  ...
  const onSubmit = async (data: UserEditData) => {
    ...
    try {
      ...
      if (data.profile) {
        formData.append("profile", data.profile); // must match backend field
      }
    ...
    <div className="max-w-lg rounded-xl border border-emerald-100 bg-white p-6 shadow-sm">
      ...
      <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
          ...
          <input
            {...register("username")}
            className="w-full rounded-md border border-sky-200 bg-sky-50 px-3 py-2 text-sm text-black placeholder:text-gray-400 outline-none focus:border-sky-400"
          />
          {errors.username && (
            <p className="text-xs text-red-600">
